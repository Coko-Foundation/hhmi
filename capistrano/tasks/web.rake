...
namespace :deploy do
  ...
  desc "Restart dev node server"
  task :restart_node_dev do
    on roles(:web) do
      execute :salt, '-N', "node-dev", 'service.restart', "node-dev-assessmentbuilder_node"
    end
  end

  desc "Restart stage node server"
  task :restart_node_stage do
    on roles(:web) do
      execute :salt, '-N', "node-stage", 'service.restart', "node-stage-assessmentbuilder_node"
    end
  end

  desc "Restart prod node server"
  task :restart_node_www do
    on roles(:web) do
      execute :salt, '-N', "node-www", 'service.restart', "node-www-assessmentbuilder_node"
    end
  end
end
